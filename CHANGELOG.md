...
- **秘密値がプロセス引数に出なくなった**（Codex 独立レビュー MEDIUM 指摘）。従来は macOS が `security ... -w <値>`、Windows が `cmdkey ... /pass:<値>` で保存していたため、子プロセスが生きている間、**FTP パスワードと（v0.13.1 以降は）AES バックアップ鍵をプロセス一覧から読み取れた**。
  - **macOS**: `security -i`（対話モード）へ変更し、コマンド行ごと stdin で渡す。argv に出るのは `-i` だけ。
    - `-w` を値なしにするプロンプト方式は**採用しなかった** — macOS 25.6 での実測で **128 バイトを超える値が黙って切り捨てられる**（1024 文字のパスワードが 89 文字になった）。誤ったパスワードをエラーなしで保存してしまう
    - `-i` は引数を解釈するため、service / account を**引用**し、引用符・バックスラッシュ・制御文字を含む値は**保存前に拒否**する（引数インジェクション防止）
...
- **拡張の表示名からパス区切り文字を除去（有効化に失敗する問題）** — Claude Desktop は拡張の `display_name` を MCP サーバの識別子としてパスの一部に使う。表示名に含めていたスラッシュ（`FTP/FTPS`）がパス脱出ガードに引っかかり、有効化すると **「拡張機能サーバーに接続できません」** と表示され、ログに `path escape: "aiftp — FTP/FTPS 安全デプロイ"` が記録されていた。実際にはリトライで接続が成立するため機能はしていたが、利用者は最初の画面で手が止まる。区切りを中黒に変え（`aiftp — FTP・FTPS 安全デプロイ`）、表示名にパス区切り文字が入ったら失敗する回帰テストを追加した。**macOS / Windows の両方で発生する**（2026-08-15 の Windows 実機ドッグフードで発見・修正後に同一機で解消を確認。当初は「macOS はスラッシュを内部で読み替えるため再現しない Windows 固有の不具合」と記録していたが、2026-09-05 に macOS の Claude Desktop ログで同一エラーを実測したため訂正）。
