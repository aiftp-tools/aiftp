...
## macOS `security` の実測値（2026-09-07）

- 🔴 **`-w` を値なしにするプロンプト方式は 128 バイトで黙って切り捨てる** — 1024 文字のパスワードが 89 文字で保存された（エラーなし）。秘密値を stdin で渡す目的では**使えない**
- ✅ **`security -i`（対話モード）には長さ制限がない** — 1377 文字を往復で完全一致。内側コマンドの失敗も終了コードに伝播する（usage=2 / 重複=45 を実測）
- ⚠️ **`-i` は引数を解釈する** — argv 経由（`execFile`）にはなかった**引数インジェクションの経路が生まれる**。service / account は必ず二重引用する（引用内なら `-A -T /bin/sh` も値の一部として扱われることを実測。`security` はシェルではないので `$(...)` やバッククォートの展開はない）
- 🔴 **`-i` は二重引用の中でもバックスラッシュをエスケープとして解釈する** — `DOMAIN\user` を素で渡すと **`DOMAINuser` に化けて読み戻せなくなる**（実測）。`\` → `\\`、`"` → `\"` にエスケープすること。**拒否ではなくエスケープ**（拒否すると `DOMAIN\user` 形式の FTP ユーザーが使えなくなる）
